...
export class JssGraphQLService {
   ...
   */
  query<T, V = R>(options: QueryOptions & any & JssGraphQLOptions): Observable<ApolloQueryResult<T>> {
   ...
   */
  mutate<T, V = R>(options: MutationOptions & any & JssGraphQLOptions): Observable<FetchResult<T>> {
    if (this.isEditingOrPreviewingAndSsr) {
      return empty();
    }
